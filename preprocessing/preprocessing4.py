...
for insurance_id in U.keys():
    ...
    if insurance_id not in [1202130601]:
        try:
            with open('./R_%d.temp' % insurance_id, 'rb') as fp:
                R = pickle.load(fp)
                fp.close()
                
            with open('./basket_%d.temp' % insurance_id, 'rb') as fp:
                temp = pickle.load(fp)
            ...
            with open('./collection_tweets_%d.temp' % insurance_id, 'rb') as fp:
                collection_of_tweets = pickle.load(fp)
                fp.close()
                
            m = json.dumps({'message': 'Working',\
                            'place_at': 'Found temp files'}),\
            ...
            # Saves root and derivations for this account
            with open('R_%d.temp' % insurance_id, 'wb') as fp:
                pickle.dump(R, fp)
                fp.close()
            
            with open('basket_%d.temp' % insurance_id, 'wb') as fp:
                pickle.dump(basket, fp)    
                fp.close()
            
            with open('collection_tweets_%d.temp' % insurance_id, 'wb') as fp:
                pickle.dump(collection_of_tweets, fp)
                fp.close()
            
            # 5. Genarates the output file in document_idXtoken format
            # where document_id ε collection and token ε t2  
            m = ('Generating dataset file and saving R dict', \
                        time.asctime(time.localtime(time.time())))
            logging.info(m)
        ...
        print '%d already preprocessed' % insurance_id
        m = ('%d already preprocessed' % insurance_id, time.asctime(time.localtime(time.time())))
        logging.info(m)
